    ...
    : App(spWindow)
{
	...
	spEntity->SetShaderId(*spEngine->GetShaderManager()->AddShader(std::filesystem::path(SHADERS_DIR)/"DemoTriangleRGB.vert", std::filesystem::path(SHADERS_DIR)/"DemoTriangleRGB.frag"));
}
