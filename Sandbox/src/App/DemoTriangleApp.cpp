    ...
    : App(pWindow)
{
	...
	spEntity->SetShaderId(spEngine->GetShaderManager()->AddShaderFromFiles(std::filesystem::path(SHADERS_DIR)/"DemoTriangle.vert", std::filesystem::path(SHADERS_DIR)/"DemoTriangle.frag"));
