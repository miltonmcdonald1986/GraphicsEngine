...
auto CreateEntityFactory(const IEnginePtr& spEngine) -> IEntityFactoryPtr {
  return std::make_shared<EntityFactory>(spEngine);
}

EntityFactory::EntityFactory(IEnginePtr spEngine) : m_spEngine(std::move(spEngine)) {}

auto EntityFactory::AddCube(const attributes::Attributes& attributes)
    -> Entity* {
  // Each face is made of two triangles with vertices determined by the
  // following indices
