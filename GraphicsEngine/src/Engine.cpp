...
Engine::Engine(const std::optional<GLProcAddressFunc>& proc_address_func)
    : m_upEntityManagerImpl(entities::CreateEntityManagerImpl(this)),
      ...
      m_spLog(CreateLog()) {
  if (proc_address_func) {
    if (gladLoadGLLoader(*proc_address_func) == 0)
...
Engine::~Engine() { spdlog::shutdown(); }

auto Engine::CreateNewTextureFromFile(std::string_view textureName,
...
auto Engine::GetCamera() const -> ICameraPtr { return m_spCamera; }

auto Engine::GetEntityManager() -> entities::EntityManager* {
  return m_upEntityManagerImpl.get();
}

auto Engine::GetLog() const -> ILogPtr { return m_spLog; }
...
auto Engine::Render() -> void {
  ...
  for (auto entId : m_upEntityManagerImpl->GetEntityIds()) {
    ...
    for (size_t i = 0; i < textures.size(); ++i) {
      ...
      pEntity->shader->SetUniformData(spTexture->GetName(),
                                      static_cast<int>(i));
    }
    GL::BindVertexArray(pEntity->kEntityId);
    if (pEntity->kNumIndices > 0)
      GL::DrawElements(GL_TRIANGLES, pEntity->kNumIndices, GL_UNSIGNED_INT,
                       nullptr);
    else
...
auto Engine::SetBackgroundColor(const Types::Color& color) -> void {
  ...
  GL::ClearColor(m_BackgroundColor.r, m_BackgroundColor.g, m_BackgroundColor.b,
                 m_BackgroundColor.a);
}

auto Engine::SetCamera(ICameraPtr spCamera) -> void { m_spCamera = spCamera; }
