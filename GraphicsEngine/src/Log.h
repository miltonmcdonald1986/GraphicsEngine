...
class queue_sink : public spdlog::sinks::base_sink<std::mutex> {
 ...
 protected:
  void sink_it_(const spdlog::details::log_msg& msg) override {
    ...
    if (static_cast<int>(messages_.size()) > m_MaxNumMessages) messages_.pop();
  }
 ...
 public:
  ...
  [[nodiscard]] auto get_messages() const -> Types::Strings {
...
class Log : public ILog {
 public:
  ...
  auto Trace(std::string_view message, const std::source_location& loc) const
      -> void override;
  auto Debug(std::string_view message, const std::source_location& loc) const
      -> void override;
  auto Info(std::string_view message, const std::source_location& loc) const
      -> void override;
  auto Warn(std::string_view message, const std::source_location& loc) const
      -> void override;
  auto Error(std::string_view message, const std::source_location& loc) const
      -> void override;
  auto Critical(std::string_view message, const std::source_location& loc) const
      -> void override;
  auto SetLevel(LogLevel level) -> void override;

 private:
  auto LogMessage(LogLevel level, const std::source_location& loc,
                  std::string_view message) const -> void;

  std::shared_ptr<spdlog::logger> m_spLogger = nullptr;
  std::shared_ptr<spdlog::sinks::rotating_file_sink_mt> m_spRotatingFileSink =
      nullptr;
