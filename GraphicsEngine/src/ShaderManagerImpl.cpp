...
ShaderManagerImpl::ShaderManagerImpl(IEngine* pEngine) : m_pEngine(pEngine) {}

auto ShaderManagerImpl::AddShaderFromFiles(const Types::Path& vert,
  ...
  std::optional<Types::String> oGeomSource =
      oGeom ? std::optional<Types::String>(
                  ShaderUtilities::GetSourceFromFile(*oGeom))
            : std::nullopt;
...
auto ShaderManagerImpl::AddShaderFromSource(const Types::StringView& vert,
