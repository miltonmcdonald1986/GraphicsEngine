...
auto CompileShadersAndLinkProgram(Types::StringView vert,
...
auto InitShader(Types::StringView vert, Types::StringView frag,
                Types::OptStringView oGeom)
    -> std::tuple<Types::StringView, Types::OptStringView, Types::ShaderId,
                  Uniforms, Types::StringView>;

auto LinkProgram(const std::vector<GLuint>& shaders) -> GLuint;

auto SetUniformData(Types::ShaderId id, Uniform& uniform,
                    const Types::UniformData& data) -> bool;
...
auto CompileShader(GLenum shaderType, const char* source) -> GLuint {
  ...
  if (!success) {
    if (spLog) spLog->Error("Error when compiling shader.");
...
auto CompileShadersAndLinkProgram(Types::StringView vert,
                                  ...
                                  std::optional<Types::StringView> oGeom)
    -> Types::Id {
  auto vertShaderId = CompileShader(
      GL_VERTEX_SHADER, vert.empty() ? nullptr : std::string(vert).c_str());
  auto geomShaderId = CompileShader(
      GL_GEOMETRY_SHADER, oGeom ? std::string(*oGeom).c_str() : nullptr);
  auto fragShaderId = CompileShader(
      GL_FRAGMENT_SHADER, frag.empty() ? nullptr : std::string(frag).c_str());
  auto id = LinkProgram({vertShaderId, geomShaderId, fragShaderId});
  if (id == 0) GetLog()->Error("Failed to create shader program.");
...
auto GetKeys(const Uniforms& uniforms) -> Types::StringViews {
  ...
  std::ranges::transform(
      uniforms, std::back_inserter(keys),
      [](const std::pair<Types::StringView, Uniform>& uniform) {
...
/// output tuple is 0: fragment Source, 1: optional geometry source, 2: shader
/// id, 3: uniform names, and 4: vertex source
auto InitShader(Types::StringView vert, Types::StringView frag,
                Types::OptStringView oGeom)
    -> std::tuple<Types::StringView, Types::OptStringView, Types::ShaderId,
                  Uniforms, Types::StringView> {
  ...
  for (int i = 0; i < count; ++i) {
    ...
    switch (type) {
      ...
      default: {
        GetLog()->Warn(
            std::format("Shader uniform type not handled: {}", type));
        break;
      }
    }
...
auto LinkProgram(const std::vector<GLuint>& shaders) -> GLuint {
  ...
  for (auto shader : shaders) {
    if (shader != 0) GL::AttachShader(shaderProgram, shader);
  }
  ...
  if (!success) {
    ...
    std::string infoLog(infoLogLength, '\0');
    GL::GetProgramInfoLog(shaderProgram, infoLogLength, nullptr,
                          infoLog.data());
...
auto SetUniformData(Types::ShaderId id, Uniform& uniform,
                    const Types::UniformData& data) -> bool {
  if (!GL::IsProgram(id)) return false;

  uniform.SetData(data);

  const auto location = uniform.GetLocation();
  GL::UseProgram(id);
  if (std::holds_alternative<float>(data))
    GL::Uniform1f(location, std::get<float>(data));
  else if (std::holds_alternative<glm::mat4x4>(data))
    GL::UniformMatrix4fv(location, 1, GL_FALSE,
                         glm::value_ptr(std::get<glm::mat4>(data)));
...
ShaderImpl::ShaderImpl(Types::StringView vert, Types::StringView frag,
                       Types::OptStringView oGeom)
    : ShaderImpl(InitShader(vert, frag, oGeom)) {}

ShaderImpl::ShaderImpl(
    const std::tuple<Types::StringView, Types::OptStringView, Types::ShaderId,
                     Uniforms, Types::StringView>& data)
    : Shader(std::make_tuple(std::get<0>(data), std::get<1>(data),
                             std::get<2>(data), GetKeys(std::get<3>(data)),
                             std::get<4>(data))),
      m_Uniforms(Uniforms(std::get<3>(data))) {}

auto ShaderImpl::GetUniformData(Types::StringView name) const
    -> std::optional<Types::UniformData> {
  auto itUniform = m_Uniforms.find(name);
  if (itUniform == m_Uniforms.end()) return std::nullopt;

  return itUniform->second.GetData();
}

auto ShaderImpl::SetUniformData(Types::StringView name,
                                const Types::UniformData& data) -> bool {
  auto itUniform = m_Uniforms.find(name);
  if (itUniform == m_Uniforms.end()) return false;
